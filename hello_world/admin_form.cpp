...
{
    int legindex = ui->leagues_edit_goals->currentIndex();
    int tmindex = ui->Teams_editgoals->currentIndex();
    int plindex = ui->player_editgoals->currentIndex();

    adm.leagues[legindex].teams[tmindex].delete_specific_player(plindex);
...
{
    adm.add_league(ui->league_new_name->text().toStdString(),adm.get_league_id());
    home_labels_edit();
}
...
void admin_form::on_pushButton_2_clicked()
{
    ...
    Team newT(ui->new_team_name->text().toStdString(),adm.get_team_id(),players);
...
void admin_form::on_pushButton_3_clicked()
{
    ...
    if (outFile.is_open())
    {
         ...
         outFile << adm.get_player_an_id()<<std::endl;
         outFile << adm.get_team_id()<<std::endl;
         outFile << adm.get_league_id()<<std::endl;
               ...
               // Team team(teamName, teamId);
                ...
                for (int k = 0; k < 11; k++) {
                   ...
                   // int playerId, price;
                    ...
                    map_player_admin[adm.leagues[i].teams[j].players[k].player_id]=adm.leagues[i].teams[j].players[k];
...
void admin_form::on_NEXt_GW_clicked()
{
        ...
        player pdefault;
        map_player_admin[pdefault.player_id]=pdefault;

        //claculate all users points for this gw
        MainWindow m;
        m.calc_points(adm.gw_num,map_player_admin);

        //then save the data to users file
        m.save_data();
